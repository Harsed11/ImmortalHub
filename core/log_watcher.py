...
class DotaLogWatcher:
    ...
    def find_log_files(self) -> List[str]:
        ...
        for folder in ['dota', 'dota_russian', '']:
            for name in ['console.log', 'server_log.txt', 'connection.log']:
                p = os.path.join(self.dota_path, folder, name) if folder else os.path.join(self.dota_path, name)
                if os.path.exists(p):
                    candidates.append(p)
        return candidates
    ...
    def scan_now(self) -> List[int]:
        ...
        for fpath in log_files:
            try:
                ...
                logger.debug(f'Error reading log file {fpath}: {e}')
